...
def lines(text:str,width:int): return textwrap.wrap(text,width=width,break_long_words=False)
...
def save(img,path): img.save(path,"JPEG",quality=94,optimize=True)
def canvas(): return Image.new("RGB",(W,H),WHITE)

def cover(lesson,data,path):
    im=canvas(); d=ImageDraw.Draw(im); d.rectangle((0,0,W,H),fill=NAVY); d.rectangle((0,700,W,H),fill=BLUE)
    d.rectangle((55,58,1025,140),fill=WHITE); d.text((90,75),"COMMERCIAL INVESTOR ACADEMY",font=F_H2,fill=NAVY)
    d.rounded_rectangle((65,205,315,290),radius=14,fill=RED); d.text((95,220),f"LESSON {lesson}",font=F_H2,fill=WHITE)
    y=draw_wrap(d,(65,350),data["lesson_title"].upper(),F_TITLE,WHITE,22,14); d.rectangle((65,y+16,220,y+25),fill=RED)
    draw_wrap(d,(65,y+65),data["subtitle"],F_BODY,WHITE,38)
    for i,h in enumerate([210,320,250,390,285,350,230]):
        x=80+i*135; d.rectangle((x,920-h,x+95,920),fill=(215,224,238))
        for wx in range(x+14,x+80,28):
            for wy in range(940-h,900,38): d.rectangle((wx,wy,wx+12,wy+18),fill=BLUE)
    footer(d); save(im,path)

def definition(data,path):
    im=canvas(); d=ImageDraw.Draw(im); header(d,2,"What Is It?")
    d.ellipse((365,190,715,540),fill=NAVY); d.text((470,290),"$",font=get_font(130,True),fill=WHITE)
    draw_wrap(d,(95,620),data["definition"],F_BODY,GRAY,45,14)
    d.rounded_rectangle((95,960,985,1110),radius=24,fill=(231,239,250)); d.text((135,1000),"Investor takeaway:",font=F_BODY_B,fill=NAVY)
    draw_wrap(d,(465,1000),data["subtitle"],F_SMALL_B,RED,36,9); footer(d); save(im,path)

def listslide(n,title,items,path,pos=True):
    im=canvas(); d=ImageDraw.Draw(im); header(d,n,title); y=210; c=GREEN if pos else RED; s="✓" if pos else "×"
    for item in items:
        d.ellipse((85,y,145,y+60),fill=c); d.text((101,y+4),s,font=F_H2,fill=WHITE); d.text((185,y+8),item,font=F_BODY,fill=GRAY)
        d.line((185,y+72,960,y+72),fill=(210,215,220),width=2); y+=145
    footer(d); save(im,path)

def example(data,path):
    im=canvas(); d=ImageDraw.Draw(im); header(d,6,data["local_example_title"]); draw_wrap(d,(80,190),data["local_example_intro"],F_BODY,GRAY,46)
    y=410
    for i,row in enumerate(data["local_example_rows"]):
        fill=NAVY if i==2 else LIGHT; tf=WHITE if i==2 else GRAY
        d.rounded_rectangle((80,y,1000,y+150),radius=18,fill=fill,outline=NAVY,width=3); d.text((125,y+50),row["label"],font=F_BODY_B,fill=tf)
        b=d.textbbox((0,0),row["value"],font=F_H2); d.text((950-(b[2]-b[0]),y+44),row["value"],font=F_H2,fill=WHITE if i==2 else RED); y+=180
    footer(d); save(im,path)

def why(data,path):
    im=canvas(); d=ImageDraw.Draw(im); header(d,7,data["why_title"]); y=210
    for item in data["why_items"]:
        d.ellipse((85,y,155,y+70),fill=NAVY); d.text((108,y+8),"✓",font=F_H2,fill=WHITE); draw_wrap(d,(195,y+8),item,F_BODY,GRAY,40); y+=160
    d.rounded_rectangle((75,930,1005,1135),radius=24,fill=(231,239,250)); d.text((115,965),"LOCAL INVESTOR QUESTION",font=F_SMALL_B,fill=NAVY)
    draw_wrap(d,(115,1015),data["engagement_question"],F_SMALL,GRAY,62,8); footer(d); save(im,path)

def main():
    topics=json.loads(TOPICS_FILE.read_text()); st=state(); idx=(st["last_index"]+1)%len(topics); lesson=st["last_lesson"]+1
    data=generate(lesson,topics[idx]); folder=OUTPUT_ROOT/f"lesson-{lesson:03d}"; folder.mkdir(parents=True,exist_ok=True)
    ...
    manifest={"lesson":lesson,"topic":topics[idx],"created_at":datetime.now(timezone.utc).isoformat(),"caption":caption,"engagement_question":data["engagement_question"],"files":urls}
    (folder/"manifest.json").write_text(json.dumps(manifest,indent=2)); (folder/"caption.txt").write_text(caption); (OUTPUT_ROOT/"latest.json").write_text(json.dumps(manifest,indent=2))
    STATE_FILE.parent.mkdir(parents=True,exist_ok=True); STATE_FILE.write_text(json.dumps({"last_index":idx,"last_lesson":lesson},indent=2))
